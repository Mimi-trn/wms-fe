...
@Component({
  selector: 'app-GRPO',
  templateUrl: './GRPO.component.html',
  styleUrls: ['./GRPO.component.css'],
})
export class GRPOComponent implements OnInit {
  ...
  async getData(page: any, per_page: any) {
    if (this.choose == 1) {
      ...
      if (resp.result.responseCode == '00') {
        this.loading.stop();
        this.datas = resp.data.map((obj: any) => ({
          ...obj,
        }));
        this.total = resp.dataCount;
      } else {
  ...
  ngOnInit() {
    ...
    this.listStatus = [
      ...
      { text: 'Nhập kho đã ký xác nhận', value: 2 },
    ];
    ...
    const columnIqc: any = sessionStorage.getItem('columns-GRPO');
    this.columns = columnIqc
      ...
      : [
        ...
        {
          keyTitle: 'ui.receipt.GRN.supplier',
          keyName: 'vendorName',
          width: '200px',
          check: false,
          count: 0,
  ...
  onClickCheckBox(event: any) {
    sessionStorage.setItem('columns-GRPO', JSON.stringify(this.columns));
  }
