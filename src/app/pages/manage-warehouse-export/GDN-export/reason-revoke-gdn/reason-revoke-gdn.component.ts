...
@Component({
  selector: 'app-reason-revoke-gdn',
  templateUrl: './reason-revoke-gdn.component.html',
  styleUrls: ['./reason-revoke-gdn.component.css'],
})
export class ReasonRevokeGdnComponent implements OnInit {
  ...
  async confirm() {
    if (!this.reason) {
      ...
      let resp = await this.reasonService.createRevokeGDN(
        this.child,
        this.reason
      );
