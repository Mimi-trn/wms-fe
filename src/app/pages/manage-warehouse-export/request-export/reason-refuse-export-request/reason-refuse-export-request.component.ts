...
@Component({
  selector: 'app-reason-refuse-export-request',
  templateUrl: './reason-refuse-export-request.component.html',
  styleUrls: ['./reason-refuse-export-request.component.css'],
})
export class ReasonRefuseExportRequestComponent implements OnInit {
  ...
  async confirm() {
    if (!this.reason) {
      this.messageService.warning(
        `Bạn phải nhập lý do từ chối yêu cầu xuất kho`,
        `Cảnh báo`
      );
    } else {
      let resp = await this.reasonService.createReasonRefuseExportRequest(
        this.child,
        this.reason
      );
